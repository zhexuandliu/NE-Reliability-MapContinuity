...
#' @examples
#' get_loss_pointwise_tsne(yy, Y, Ydist_sq, Mat)
#' @export
#' @import Rfast
#' @import RtsneWithP
#' @export
...
gradient_compute_pointwise = function(yy, Y, P){
  ...
  I2 = (-1) * 4 * yy_Y_diff / deno / ((1 + YDiffDistSq_double) ^ 2) * sum(P)
...
perturbation_score_compute = function(i, X, Y, Ydist_sq = NULL, perplexity, dir_vec, length, initial_dims = 50, PCA_result = NULL, approx = 0){
  if (approx == 0){
    ...
    for (direction in dir_vec) {
      ...
      for (init_ind in initializations_ind) {
        ...
        OPT = optim(Y_new_init, fn = function(x){
          get_loss_pointwise_tsne(x, Y[-i,], Ydist_sq[-i,][,-i], P_new)},
    ...
    for (direction in dir_vec) {
      ...
      best_result = NULL
      initializations_ind =
        c(i, order(P_new[1,], decreasing = TRUE)[1])
      print(Sys.time())
      for (init_ind in initializations_ind) {
        if (init_ind < i) {
          Y_new_init = Y[init_ind - 1,]
    ...
    for (direction in dir_vec) {
      ...
      print(Sys.time())
      P_new = P_new$P
      Y_new = Y[order(P_new[1,], decreasing = TRUE)[1],]
      PScoreVec[dir_id] = sum((Y[i,] - Y_new) ^ 2) ^ 0.5
      dir_id = dir_id + 1
      print(Sys.time())
    }
